...
import { Alert, FlatList, StyleSheet, Text, View, TouchableOpacity, ListRenderItemInfo } from 'react-native';
...
export function HomePage({ navigation }: NavigationProps) {
  ...
  return (
    <Container>
      <ListaRefeicoes data={data} navigation={navigation} />
    </Container>
  );
}
...
function ListaRefeicoes(props: ListaRefeicoesProps) {
  const { data, navigation } = props;

  return (
    <FlatList
      data={data}
      renderItem={(item) => <ItemListaRefeicoes item={item} navigation={navigation} />}
      keyExtractor={item => item.id.toString()}
    />
  );
}

interface ItemListaRefeicoesProps {
  item: ListRenderItemInfo<ListaRefeicaoTypes>;
  navigation: NativeStackNavigationProp<RootStackParamList>;
}

function ItemListaRefeicoes(props: ItemListaRefeicoesProps) {
